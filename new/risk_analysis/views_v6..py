...
current_file = '/tmp/ndpi_temp1.json'  # Initialize to avoid None
capture_start_time = None  # Tracks start timestamp
captured_flows = []  # Collects flows between start and stop
...
def run_capture():
    ...
    while not capture_stop_event.is_set():
        ...
        output_file = file1 if round_count % 2 == 1 else file2
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if os.path.exists(output_file):
                    ...
                    logger.warning(f"Round {round_count}: {output_file} does not exist after capture")
            else:
...
def start_capture(request):
    """Start the capture thread and record start time."""
    global capture_thread, capture_start_time, captured_flows
    if request.method == 'POST':
        if capture_thread is None or not capture_thread.is_alive():
            capture_stop_event.clear()
            capture_start_time = datetime.now().astimezone()
            captured_flows = []  # Reset captured flows
            capture_thread = threading.Thread(target=run_capture, daemon=True)
            capture_thread.start()
            logger.info(f"Started nDPI capture thread at {capture_start_time}")
...
def stop_capture(request):
    """Stop the capture thread and save captured flows to captured.json."""
    global capture_thread, capture_start_time, captured_flows
    if request.method == 'POST':
        if capture_thread and capture_thread.is_alive():
            ...
            if capture_thread.is_alive():
                logger.warning("Capture thread did not terminate within 5 seconds")
            else:
                logger.info("Capture thread stopped successfully")
            capture_thread = None

            # Save captured flows to captured.json
            log_dir = '/root/Project2/logs'
            log_file = os.path.join(log_dir, 'captured.json')
            try:
                os.makedirs(log_dir, exist_ok=True)
                with open(log_file, 'w') as f:
                    json.dump(captured_flows, f, indent=2)
                logger.info(f"Saved {len(captured_flows)} flows to {log_file}")
            except Exception as e:
            ...
            captured_flows = []
            return JsonResponse({'status': 'success', 'message': 'Capture stopped and flows saved successfully.'})
        else:
...
def get_latest_flows(request):
    """Read flows from the non-writing file, process, and append to captured_flows."""
    global captured_flows
    if request.method == 'GET':
        file1 = '/tmp/ndpi_temp1.json'
        file2 = '/tmp/ndpi_temp2.json'
        file_to_read = file2 if current_file == file1 else file1
        logger.debug(f"Reading flows from {file_to_read}")
        ...
        try:
            with open(file_to_read, 'r') as f:
                ...
                logger.debug(f"Raw content of {file_to_read}: {content[:500]}...")
                try:
                    ...
                    for packet in packet_data:
                        try:
                            ...
                            if capture_start_time is not None:
                                captured_flows.append(packet_info)
...
def risk_analysis_view(request):
        ...
        if packet_file:
            try:
                ...
                if not packet_data:
                    ...
                    cutoff_time = datetime.now().astimezone() - timedelta(minutes=10)
    ...
    # Filter packets to last 10 minutes for interface
    cutoff_time = datetime.now().astimezone() - timedelta(minutes=10)
    packets = PacketAnalysis.objects.filter(first_seen__gte=cutoff_time).order_by('-first_seen')
    logger.debug(f"Initial packets fetched: {packets.count()} packets")
