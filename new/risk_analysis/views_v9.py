...
def add_protocol_page(request):
    """Render the add_protocol.html page."""
    return render(request, 'risk_analysis/add_protocol.html')

@csrf_exempt
def add_protocol(request):
    """Handle adding a custom protocol to protos.txt and verify with ndpiReader."""
    if request.method == 'POST':
        ...
        # Mandatory fields
        if not value or not protocol_name:
            return JsonResponse({'error': 'Value and protocol name are required'}, status=400)
        ...
        if format_type in ['ip', 'ipv6', 'ip_hostname']:
            for i, line in enumerate(lines):
                if ip_section_marker in line:
                    # Find the end of the IP section (next section or blank line)
        ...
        # Run ndpiReader to capture flows
        ...
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
            logger.info(f"ndpiReader output: {result.stdout}")
            if os.path.exists(json_output):
                ...
                logger.warning(f"No flows captured at {json_output}")
        except subprocess.CalledProcessError as e:
            logger.error(f"ndpiReader failed: {e.stderr}")
            return JsonResponse({'error': f'ndpiReader failed: {e.stderr}', 'success': True}, status=500)
        except subprocess.TimeoutExpired:
            logger.error("ndpiReader timed out")
            return JsonResponse({'error': 'ndpiReader timed out', 'success': True}, status=500)
        except Exception as e:
            logger.error(f"Error running ndpiReader: {str(e)}")
            return JsonResponse({'error': f'Error running ndpiReader: {str(e)}', 'success': True}, status=500)
