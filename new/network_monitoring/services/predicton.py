    ...
    def predict_next_values(self, lte_type):
        ...
        try:
            ...
            if df is None or df.empty:
                logger.warning(f"Skipping prediction: Insufficient data for {lte_type}")
                return None
            ...
            if RFParameters.objects.filter(timestamp=next_timestamp, lte_type=lte_type).exists():
                logger.info(f"Prediction already exists for {lte_type} at {next_timestamp}")
                return None

            # Preprocess data
            input_data = self.preprocess_data(df)
            if input_data is None:
                logger.error(f"Skipping prediction: Preprocessing failed for {lte_type}")
                return None
            ...
            # Fetch last actual values
            last_actual = df.iloc[0][['rsrp', 'rsrq', 'sinr']].values.reshape(1, -1)

            # Blend prediction with actual data for smoother transitions
