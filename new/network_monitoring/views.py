...
def save_config(request):
    ...
    try:
        ...
        # Validate latency and jitter thresholds if enabled
        latency_enabled = data.get("latency_enabled", False)
        jitter_enabled = data.get("jitter_enabled", False)
        
        if latency_enabled and (
            "latency_threshold" not in data or 
            not str(data["latency_threshold"]).replace(".", "", 1).isdigit() or 
            float(data["latency_threshold"]) <= 0
        ):
            return JsonResponse({
                "status": "error",
        ...
        if jitter_enabled and (
            "jitter_threshold" not in data or 
            not str(data["jitter_threshold"]).replace(".", "", 1).isdigit() or 
            float(data["jitter_threshold"]) <= 0
        ):
            return JsonResponse({
                "status": "error",
        ...
        config = {
            ...
            "latency_enabled": bool(data.get("latency_enabled", False)),
            "latency_threshold": float(data["latency_threshold"]) if latency_enabled else 100.0,
            "jitter_enabled": bool(data.get("jitter_enabled", False)),
            "jitter_threshold": float(data["jitter_threshold"]) if jitter_enabled else 50.0
        }
...
# Function to clean latency/jitter values with potential 'ms' suffix
...
def apply_switching_logic():
    """
    Enhanced Switching Logic that uses loss_per_instance, latency, and jitter thresholds
    
    Key Steps:
    1. Read thresholds from config
    2. Calculate average packet loss 
    3. Check latency and jitter against thresholds
    4. Determine PRL, LAT, and JIT flags based on measurements
    5. Combine flags for final switching decision
    """
    config = load_config()
    n = config["num_test_instances"]
    ...
    # Get latency and jitter configuration
    latency_enabled = config.get("latency_enabled", False)
    latency_threshold = config.get("latency_threshold", 100.0)
    jitter_enabled = config.get("jitter_enabled", False)
    jitter_threshold = config.get("jitter_threshold", 50.0)
    ...
    for lte_type in ["LTE1", "LTE2"]:
        ...
        avg_latency = sum(clean_measurement(record.latency) for record in packet_loss_records) / len(packet_loss_records) if packet_loss_records else 0

        avg_jitter = sum(clean_measurement(record.jitter) for record in packet_loss_records) / len(packet_loss_records) if packet_loss_records else 0


        # PRL Flag: 1 if average packet loss is greater than loss threshold, 0 otherwise
        prl_flag = 1 if avg_packet_loss > lt else 0
        
        # LAT Flag: 1 if latency_enabled and average latency is greater than threshold, 0 otherwise
        lat_flag = 1 if (latency_enabled and avg_latency > latency_threshold) else 0
        
        # JIT Flag: 1 if jitter_enabled and average jitter is greater than threshold, 0 otherwise
        jit_flag = 1 if (jitter_enabled and avg_jitter > jitter_threshold) else 0
        ...
        if latest_predicted:
            ...
            # Enhanced Switching Index (SI)
            # Now includes latency and jitter flags if enabled
            # If any of the enabled metrics is bad (flag=1), consider switch
            metric_flags = [prl_flag]
            if latency_enabled:
                metric_flags.append(lat_flag)
            if jitter_enabled:
                metric_flags.append(jit_flag)
                
            # SI is 1 if LQI is bad AND any of the enabled metrics is bad
            si = 1 if (lqia == 1 and any(metric_flags)) else 0
            ...
            link_data[lte_type] = {
                ...
                "high_loss_count": len(high_loss_records)
            }
